...
public class DefaultCredentialsMatcher extends SimpleCredentialsMatcher {
     ...
     */
    private void retry(String mid) { // 用户有可能会重复的调用
        ...
        if (num.incrementAndGet() > this.max) { // 已经达到了最大的尝试次数
    ...
    public boolean doCredentialsMatch(AuthenticationToken token, AuthenticationInfo info) {
        // 此处的方法主要进行密码的匹配操作，这里面需要接收的认证的Token信息
        // 如果要进行密码尝试次数的控制，必须首先获取用户的登录id的信息
